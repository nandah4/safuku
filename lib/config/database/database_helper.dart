...
class DatabaseHelper {
  ...
  Future<void> _onUpgrade(Database db, int oldVersion, int newVersion) async {
    ...
    // Run each migration sequentially to avoid skipping versions
    for (int i = oldVersion + 1; i <= newVersion; i++) {
      AppLogger.i('Running migration to version $i');
      switch (i) {
        case 2:
  ...
  Future<void> _migrateToV2(Database db) async {
    ...
    // Create mst_category (+ unique constraint)
    await db.execute('''
...
''');
      ...
      AppLogger.e('Error creating database: $e');
      rethrow;
    }
  }
